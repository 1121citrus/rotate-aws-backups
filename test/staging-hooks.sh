...
test_staging_cron_fires() {
    ...
    if "${found}"; then
        echo "PASS '${FUNCNAME[0]}': cron rotation completed within ${elapsed}s" \
             "(DRYRUN=${DRYRUN:-true})"
    else
